...
> 原生高考 3817 词 + 小学种子 50 词 + 初中种子 100 词；  
> 纯前端、零后端、零 AI、零拼写负担；  
...
- 小学（课标 50 词种子）→ 初中（中考考纲 100 词种子）→ 高考（3817 完整词库）
- 学段间数据零污染，重置只影响当前学段
...
### 📚 完整词库
- 收录高考 3817 词，每个词包含：音标、中文释义、助记（部分）、易混词组 ID
- 双向搜索（英文 / 中文）
...
│   └── senior.ts               # 高考 3817 词（复用 data_chunk1/2/3）
├── utils/                      # 纯函数工具集
│   ├── parser.ts               # 词汇解析
│   ├── array.ts                # 数组工具
│   ├── quiz.ts                 # 测验生成
